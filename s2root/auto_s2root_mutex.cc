...
#include <iostream>  /* Nobu 2020/04/14 4:51AM */
#include <mutex> /* Nobu 2020/04/14 4:51AM */
#include <thread> /* Nobu 2020/04/14 4:51AM */
...
void WriteToDisk(TFile *file, unsigned int *loopID, std::mutex *lock) {
  if (lock->try_lock()) {
    std::cout << "WriteToDisk - we're creating the new thread in loop number " << *loopID << std::endl;
...
int main(int argc, char **argv)
{
   ...
   if (argc < 2) {
      printf("******Error in invoking s2root\n");
      printf("===>  s2root shm_name file.root [compress] [tolower] [lrecl] [bufsize] [optcwn] \n");
      printf("      i.e., s2root FRED FRED.root\n");
      printf("      if file.root is not given  it will be = shm_name.root\n");
   ...
   if (argc > 2) {
      ...
      Int_t nchf = strlen(shm_name)+strlen(".root")+1;
      file_out= new char[nchf];
      strlcpy(file_out,shm_name,nchf);
...
--> End */
   ...
   char file_out_temp[2000];
   snprintf(file_out_temp,2000,"%s_pid%d.root",shm_name,process_id);
   char root_file_title[2000];
   snprintf(root_file_title,2000,"HBOOK in the shared memory: %s converted to ROOT",shm_name);
   TFile* f= TFile::Open(file_out_temp,"RECREATE",root_file_title,compress);
   if (!f) {
...
void convert_1d(Int_t id)
{
   ...
   chtitl[4*nwt] = 0;
   static TList tl;
   TH1F *h1 = (TH1F*)tl.FindObject(idname);
   if (!h1){
   ...
   if (hcbits[5]) {
      ...
      /*h1->SetTitle(chtitl);
	h1->SetBins(ncx,xbins);*/
      delete [] xbins;
   } else {
     /* h1 = new TH1F(idname,chtitl,ncx,xmin,xmax); */
     /*h1->SetTitle(chtitl);
       h1->SetBins(ncx,xmin,xmax);*/
   }
...
void convert_2d(Int_t id)
{
   ...
   chtitl[4*nwt] = 0;
   static TList tl;
   TH2F *h2 = (TH2F *)tl.FindObject(idname);
   if (!h2){
   ...
   /*h2->SetTitle(chtitl);
     h2->SetBins(ncx,xmin,xmax,ncy,ymin,ymax);*/
