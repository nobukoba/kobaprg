...
#include "TMonitor.h" /* --> Nobu added 2020/04/27 */
...
int main(int argc, char **argv)
{
   ...
   printf("Usage: s2root_server [port] [compress] [tolower] [lrecl] [bufsize] [optcwn] \n");
   printf("      port     = 9090 by default for TServerSocket\n");
   ...
   printf("      for cwn ntuple only: optcwn  = 1 (default)\n");
   printf("      1-byte int -> char, 2-byte int -> short,\n");
   printf("      (use 0 to keep 4-byte int) \n");
   printf("\n");
   printf("Now s2root_server2 starts!\n");
   lq = &pawc[9];
   iq = &pawc[17];
   void *qq = iq;
   q = (float*)qq;
...
--> End */
   ...
   if (!ss->IsValid()) {
     Error("s2root_server","TServerSocket(%d, kTRUE) is not valid\n", port);
     return 1;
   }
   ...
   while (1) {
	 ...
	 continue;
       ...
       TString shm_name;
       mess->ReadTString(shm_name);
       mess2.Reset(kMESS_ANY);              // re-use TMessage object
       if ((shm_name.Length()==0)||(shm_name.Length()>4)){
	 mess2.WriteInt(0);
	 s->Send(mess2);
	 continue;
       }
	 ...
	 continue;
       ...
       TString filename = Form("%s_pid%d.root",shm_name.Data(),process_id);
       TMemFile* file= new TMemFile(filename.Data(),"RECREATE",filename.Data());
       if (!file) {
	 ...
	 continue;
       ...
       file->Write();
       mess2.Reset(kMESS_ANY);              // re-use TMessage object
       mess2.WriteTString(file->GetName());
       mess2.WriteLong64(file->GetEND());   // 'mess << file->GetEND();' is broken in CINT for Long64_t
       file->CopyTo(mess2);
