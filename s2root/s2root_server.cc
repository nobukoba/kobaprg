...
--> End */
   ...
   if (!ss->IsValid()) {
     Error("s2root_server","TServerSocket(9090, kTRUE) is not valid\n");
     return 1;
   }
   ...
   while (1) {
	 ...
	 return 1;
       ...
       char file_out[2000];
       snprintf(file_out,2000,"%s_pid%d.root",shm_name,process_id);
       TMemFile* file= new TMemFile(file_out,"RECREATE",file_out);
       if (!file) {
