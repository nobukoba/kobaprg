...
Double_t real_sph_hankel_1st(Int_t n, Double_t x) {
  ...
  Double_t result_old = sqrt(2.0/(3.14159265358979323846*x))
  ...
  if (n == 0) {
    ...
    Double_t G_L   = EX*RI;
    Double_t G_L_1 = EX*(RI+RI*RI);
    Double_t G_L_2 = 0.;
    Int_t    LL2   = (n+1)-2;
    Double_t FL    = 1.0;
    for (Int_t L = 1; L <= LL2; L++){
      Double_t TWELP1 = 2.0*FL+1.0;
  ...
  std::cout << "n, x, result, retuls_old" << n
	    << ", " << x 
	    << ", " << result
	    << ", " << result_old << std::endl;
  return result;
...
private:
  Double_t FindPS(Int_t n) {
    ...
    Int_t nn = 13;
    Double_t xx = 0.1;
    std::cout << "nn, xx, SBSJN, ROOT::Math::sph_bessel(nn,xx), diff:"
	      << nn      << ", "
	      << xx << ", "
	      << ROOT::Math::sph_bessel(nn,xx) << ", "
	      << hamamoto_sph_bessel(nn,xx) << ", "
	      << ROOT::Math::sph_bessel(nn,xx)-hamamoto_sph_bessel(nn,xx)
	      << std::endl;
    std::cout << "nn, xx, SBSYN, ROOT::Math::sph_neumann(nn,xx):"
	      << nn << ", "
	      << xx << ", "
	      << ROOT::Math::sph_neumann(nn,xx) << ", "
	      << hamamoto_sph_neumann(nn,xx)  << ", "
	      << ROOT::Math::sph_neumann(nn,xx)-hamamoto_sph_neumann(nn,xx)
	      << std::endl;
  ...
  void stormer(Int_t n1, Int_t n2, Double_t **y, Double_t xx[]) {
    std::cout << "here stormer start" <<std::endl;
    ...
    if (n1 < n2) {
      ...
      for (Int_t i=0;i<5;i++) {
        y[1][n1-i] = r[n1-i]*real_sph_hankel_1st(l1,k_outer*r[n1-i]);
      }
      for (Int_t i=0;i<5;i++) {
...
void make_hamamoto_wf_comp() {
  ...
  wf[0] = new WaveFunction(2,3,(30./31.)*939.577218339,32.,0.67,1.27*pow(30,1./3.),-46.75,0.1);
