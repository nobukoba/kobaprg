...
int main(int argc, char* argv[]){
  ...
  GEBHeaderWithTimestamp   preheader;
  ...
  while (!ifs.eof()) {
    ...
    ifs.read((char*)&preheader, sizeof(preheader));
    ...
    std::vector<unsigned short> timing_marks;
    std::vector<short> wave_data;
    for (unsigned int i = 0; i < wave_bytes; i += sizeof(unsigned short)) {
      unsigned short swapped;
      ifs.read((char*)&swapped, sizeof(unsigned short));
      swapped = SwapShort(swapped);
      unsigned short mark = (swapped & 0x8000) >> 15;
      if (mark) {
    ...
    nTimingMarks            = timing_marks.size();
    for (unsigned int i = 0; i < nWaveData; i++) {
      WaveData[i] = wave_data[i];
    }
    for (unsigned int i = 0; i < nTimingMarks; i++) {
      TimingMarks[i] = timing_marks[i];
    }
    tree->Fill();
    ...
    std::cout << std::endl;*/
    if ((after_header_pos+preheader.size)==file_size) {
      std::cout << "Total event number: " << std::dec << ievent << std::endl;
      break;
    }else if((after_header_pos+preheader.size)>file_size){
      std::cout << "Total event number: " << std::dec << ievent << std::endl;
      std::cout << "The last event may be broken!" << std::endl;
      break;
    }
    ifs.seekg(after_header_pos+preheader.size);
  }
  ifs.close();
  output_file->Write();
  output_file->Close();
