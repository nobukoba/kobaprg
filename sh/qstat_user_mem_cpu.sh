...
do
    qstat -M | awk -v user_name=${user_name} 'NR>=6  && user_name==$2{gsub("mw","",$8); tot+=1; if($10=="Q"){que+=1;qmem+=$8;qcpus+=$7;}; if($10=="R"){run+=1;rmem+=$8;rcpus+=$7;}} END {printf("%-8s %3d %3d %3d %5d GB %5d GB %8d %8d\n", user_name, tot, que, run, qmem*8/1024, rmem*8/1024, qcpus, rcpus);}'
done
echo "-------- --- --- --- -------- -------- -------- --------"
