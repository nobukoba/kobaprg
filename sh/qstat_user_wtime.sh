...
arr=($(echo "${qstat_f_out}" | awk '{print $2}' | sort | uniq))
echo "Username Tot Que Run Q Wait av R Wait av"
echo "-------- --- --- --- --------- ---------"
for str in ${arr[@]}
do
    echo "${qstat_f_out}" | awk -v str=${str} '$2==str{tot+=1;if($3=="Q"){que+=1;qtime+=$7;}; if($3=="R"){run+=1;rtime+=$7;}} END {if(que>0){qave=qtime/que};if(run>0){rave=rtime/run};printf("%-8s %3d %3d %3d %3d:%02d:%02d %3d:%02d:%02d\n", str, tot, que, run, qave/3600, (qave%3600)/60, qave%60, rave/3600, (rave%3600)/60, rave%60);}'
done
echo "-------- --- --- --- --------- ---------"
