...
do
    ...
    amem=`echo "${pbsnodes_a_out}" | grep -A 15 "Mom = ${node}" | grep "resources_assigned.mem = " | awk '{sub("kb","",$3);print $3}'`
    ancpu=`echo "${pbsnodes_a_out}" | grep -A 15 "Mom = ${node}" | grep "resources_assigned.ncpus = " | awk '{print $3}'`
    ajobs=`echo "${pbsnodes_a_out}" | grep -A 15 "Mom = ${node}" | grep "jobs = " | awk '{sub("jobs = ","",$0);gsub(",","\n",$0);print $0}'| awk '{gsub("/.*","",$1);print $1}' | sort | uniq | wc -l`
    rmem=$((${tmem}-${amem}))
    rncpu=$((${tncpu}-${ancpu}))
